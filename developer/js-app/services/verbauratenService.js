const PRIME_NUMBERS = ['9999','9051','9116'];
...
const verbauratenBerechnung = async (codeBedingung, sparte,sqlString, extra, pool) => {
    try {
        ...
        // const valuesString = codeArray.map(value => `'${value}'`).join(',');
        ...
        for (let index = 0; index < codeArray.length; index++) {
            ...
            queryString = `SELECT "Verbaurate" FROM lampa."${sqlString}" WHERE "Code" = '${element}' AND "Sparte" = '${sparte}' ${extra};`;
...
const verbauratenBerechnungTeileliste = async (codeBedingung, sparte,sqlString, extra, pool) => {
    try {
        let data = codeBedingung;
        let filterString = data;
        const slashReplace = new RegExp("/", "g");
        ...
        const resultValue = parseFloat(eval(finishedString));
 ...
 * @returns {boolean}
 */
const checkForPrimeNumber = (es2) => {
    if(PRIME_NUMBERS.includes(es2)) return true;
    return false;
}

module.exports = { verbauratenBerechnung, verbauratenBerechnungTeileliste, checkForPrimeNumber };